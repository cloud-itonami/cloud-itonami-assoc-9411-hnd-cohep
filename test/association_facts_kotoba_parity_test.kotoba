(ns association-facts-kotoba-parity-test
  "The COHEP catalog in .cljc and in .kotoba, field by field.
...
(deftest a-field-the-catalog-does-not-carry-is-absent-not-blank
  ;; :last-revised-date is set on exactly one entry, and :established-date on
  ;; some but not all. If the port answered every field for every entry these
  ;; would pass while the port was wrong.
  (doseq [f ["last-revised-date" "established-date"]]
    (testing f
      (let [carried (keep-indexed (fn [i _] (when (present (call 'entry-field slug i f)) i))
                                  entries)]
        (is (= (keep-indexed (fn [i e] (when (get e (kw->field f)) i)) entries)
               (seq carried))
...
(deftest the-coverage-note-is-the-same-sentence-on-both-faces
  ;; The .cljc note is computed from the catalog, so adding an entry changes it
  ;; and the port must be regenerated with it. A note written by hand on each
  ;; side drifts silently, because nothing compares the two sentences.
  (is (= (facts/note-for slug) (present (call 'coverage-note slug)))))
